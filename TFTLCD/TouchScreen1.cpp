...
Point1::Point1(int16_t x0, int16_t y0, int16_t z0) {
  x = x0;
  y = y0;
  z = z0;
}

bool Point1::operator==(Point1 p1) {
...
Point1 TouchScreen1::getPoint(void) {
  int x, y, z;
...
TouchScreen1::TouchScreen1(uint8_t xp, uint8_t yp, uint8_t xm, uint8_t ym) {
  _yp = yp;
  _xm = xm;
  _ym = ym;
  _xp = xp;
...
int TouchScreen1::readTouchX(void) {
   pinMode(_yp, INPUT);
   pinMode(_ym, INPUT);
   digitalWrite(_yp, LOW);
   digitalWrite(_ym, LOW);
   ...
   return (analogRead(_yp));//1023-analogRead(_yp)
}


int TouchScreen1::readTouchY(void) {
   pinMode(_xp, INPUT);
   pinMode(_xm, INPUT);
   digitalWrite(_xp, LOW);
   digitalWrite(_xm, LOW);
   ...
   return (analogRead(_xm));//1023-
}


uint16_t TouchScreen1::pressure(void) {
  // Set X+ to ground
  pinMode(_xp, OUTPUT);
  digitalWrite(_xp, LOW);
